...
async function run() {
  ...
  await withServer(mkApp(), async (port) => {
    ...
    await test("login issues session + csrf cookies", async () => {
      ...
      for (const c of set) {
        if (c.startsWith("wardyn_auth=")) sessionCookie = c.split(";")[0];
        if (c.startsWith("wardyn_csrf=")) csrfCookie = c.split(";")[0].split("=")[1];
      }
    ...
    await test("session cookie POST without CSRF header -> 403", async () => {
      const r = await req(port, "POST", "/api/ping", {
        headers: { cookie: `${sessionCookie}; wardyn_csrf=${csrfCookie}` },
        body: {},
      });
    ...
    await test("session cookie POST with matching CSRF header -> 200", async () => {
      const r = await req(port, "POST", "/api/ping", {
        headers: {
          cookie: `${sessionCookie}; wardyn_csrf=${csrfCookie}`,
