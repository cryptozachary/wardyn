...
Returns quantity, position value, margin required, estimated fees.

### risk_reward
Analyze trade quality. Args: { action: "risk_reward", entryPrice: 65000, stopLoss: 63000, takeProfit: 70000, side?: "long" }
Returns R:R ratio, net ratio (after fees), required win rate, and letter grade (A-F).

### stop_loss
Optimal stop placement. Args: { action: "stop_loss", entryPrice: 65000, riskPercent?: 2, accountSize?: 10000, quantity?: 0.1, side?: "long" }
Returns percentage-based SL, fixed-risk SL, and ATR-based suggestions.
...
Returns R-multiple targets (1R, 1.5R, 2R, 3R, 5R), percentage targets, and multi-target analysis.
...
Returns breakeven price accounting for entry + exit fees and leverage.

### liquidation
Estimated liquidation price. Args: { action: "liquidation", entryPrice: 65000, leverage: 10, maintenanceMargin?: 0.005, side?: "long" }
Returns liquidation price and distance percentage. Warns if liquidation is < 5% from entry.

### compound
Compound growth projection. Args: { action: "compound", startingCapital: 10000, returnPercent: 2, trades: 100, reinvestPercent?: 100 }
Returns final capital, total return, multiplier, and milestones at key intervals.
